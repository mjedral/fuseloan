...
contract LendingPoolManager {
    ...
    if (daysElapsed > 0) {
        ...
        // Wszystkie wartości są przeskalowane o czynnik 1e18
        uint256 dailyRate = pow((1e18 + pool.interestRate), 1e18 / 365) / 1e18 - 1e18;
        // Oblicz złożone odsetki za okres
        uint256 interest = pool.totalLiquidity * (pow(1e18 + dailyRate, daysElapsed) / 1e18) - pool.totalLiquidity;
        // Zaktualizuj całkowitą płynność o naliczone odsetki
        pool.totalLiquidity += interest;
        // Zaktualizuj timestamp ostatniego obliczenia odsetek
